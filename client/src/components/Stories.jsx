...
const Stories = () => {
  ...
  const handleOpenStory = async (story) => {
    setStory(story);
    await socket.emit("story-played", {
      storyId: story._id,
  ...
  return (
    <div className="storiesContainer">
      ...
      <div
        ...
        style={isStoryPlaying ? { display: "none" } : {}}
      >
              ...
              .filter(
                (story) =>
                  ((localStorage.getItem("following") &&
                    localStorage.getItem("following").includes(story.userId)) ||
      ...
      {story && (
        <div
          className="storyPlayContainer"
          style={isStoryPlaying ? {} : { display: "none" }}
        >
          <div className="storyPlayBodyTop">
            <p>{story.username}</p>
            <span onClick={() => setIsStoryPlaying(false)}>
              <RxCross2 />
            </span>
          </div>
